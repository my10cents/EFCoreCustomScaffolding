...
namespace Microsoft.EntityFrameworkCore.Scaffolding.Internal
{
    ...
    /// </summary>
    public class FooCSharpEntityTypeGenerator : ICSharpEntityTypeGenerator
    {
        ...
        ///     directly from your code. This API may change or be removed in future releases.
        /// </summary>
        public FooCSharpEntityTypeGenerator(
            ...
            [NotNull] IEntityType entityType)
        {
            ...
            ///////////////// Garfield: public partial class SchemaEntityName
            var entityTypeName = entityType.Name;
            var entityTypeSchema = entityType.Relational().Schema;

            for (int i = 0; i < 20; i++)
                entityTypeName = entityTypeName.Replace(i.ToString(), "");

            if (entityTypeSchema != entityTypeName)
                entityTypeName = entityTypeSchema.Replace("dbo", "") + entityTypeName.Replace("Self", "");
            else
                entityTypeName = entityTypeSchema.Replace("dbo", "");
            //////////////////////////////

            _sb.AppendLine($"public partial class {entityTypeName}");
            ...
            if (collectionNavigations.Count > 0)
            {
                ...
                using (_sb.Indent())
                {
                    foreach (var navigation in collectionNavigations)
                    {
                        ...
                        var navigationTypeEntity = navigation.GetTargetType().Name;
                        var navigationTypeSchema = navigation.GetTargetType().Relational().Schema;

                        for (int i = 0; i < 20; i++)
                        {
                            navigationTypeEntity = navigationTypeEntity.Replace(i.ToString(), "");
                            navigationTypeSchema = navigationTypeSchema.Replace(i.ToString(), "");
                        }
                        if (navigationTypeSchema != navigationTypeEntity)
                            navigationTypeEntity = navigationTypeSchema.Replace("dbo", "") + navigationTypeEntity.Replace("Self", "");

                        _sb.AppendLine($"{navigationTypeEntity} = new HashSet<{navigationTypeEntity}>();");
        ...
        private void GenerateRequiredAttribute(IProperty property)
        {
            if (!property.IsNullable
                //&& property.ClrType.IsNullableType() //garfield
            ...
            [NotNull] IEntityType entityType)
        {
            ...
            if (sortedNavigations.Any())
            {
                ...
                foreach (var navigation in sortedNavigations)
                {
                    ...
                    //Garfield: ICollection<Entity> Entidy
                    var referencedTypeName   = navigation.GetTargetType().Name;
                    var referencedTypeSchema = navigation.GetTargetType().Relational().Schema.Replace("dbo", "");

                    //var navigationName = navigation.Name;
                    if (referencedTypeName != referencedTypeSchema)
                    {
                        var navigationType = navigation.IsCollection() ? $"ICollection<{referencedTypeSchema + referencedTypeName}>" : referencedTypeSchema + referencedTypeName;
                        _sb.AppendLine($"public virtual {navigationType} {referencedTypeSchema + referencedTypeName} {{ get; set; }}");
                    } else
                    {
                        var navigationType = navigation.IsCollection() ? $"ICollection<{referencedTypeName}>" : referencedTypeName;
                        _sb.AppendLine($"public virtual {navigationType} {referencedTypeName} {{ get; set; }}");
                    }
                }
            }
        }
